        ...
        <div
            ...
            <div class="flex items-center gap-5">
                <div
                    ...
                    @if ($flagUrl)
                        <img src="{{ $flagUrl }}" alt="Bendera {{ $state->state_name }}"
                            class="h-full w-full object-cover" loading="lazy"
                            onerror="
                this.style.display = 'none';
                this.nextElementSibling.style.display = 'flex';
              " />
                        <span class="hidden text-xs font-bold text-(--muted-foreground)">{{ $countryCode ?: 'N/A' }}</span>
                    @else
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <span class="rounded-full border border-black/5 px-4 py-1.5 text-xs font-semibold tracking-wide shadow-sm"
                    style="{{ $partStyle }}">
                    {{ $partLabel }}
                </span>
            ...
            <div class="space-y-6 lg:col-span-2">
                ...
                <div class="rounded-2xl border border-(--border) bg-(--card) shadow-sm">
                    ...
                    <div class="p-6">
                        ...
                        @else
                            <p class="text-sm text-(--muted-foreground) italic">Tidak ada deskripsi tersedia untuk negara
                                ini.</p>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="rounded-2xl border border-(--border) bg-(--card) shadow-sm">
                        ...
                        <div class="p-5">
                            <div class="grid grid-cols-2 gap-4">
                                <div class="rounded-lg border border-(--border) bg-(--secondary)/30 p-3 text-center">
                                    ...
                                    @endphp
                                    <p class="mt-1.5 text-sm font-bold {{ $color2013 }}">
                                ...
                                <div class="rounded-lg border border-(--border) bg-(--secondary)/30 p-3 text-center">
                                    ...
                                    @endphp
                                    <p class="mt-1.5 text-sm font-bold {{ $color2016 }}">
                            ...
                            <ul class="space-y-4">
                                @if ($state->points_of_interest)
                                    <li>
                                        ...
                                        <p class="mt-1 text-sm leading-snug font-semibold">{{ $state->points_of_interest }}
                                        </p>
                                    </li>
                                @endif
                                ...
                                @if (is_array($state->additional_info))
                                    @foreach ($state->additional_info as $key => $val)
                                        <li class="border-t border-(--border)/50 pt-2">
                                            <p class="text-xs font-medium text-(--muted-foreground)">{{ $key }}
                                            </p>
                                            <p class="mt-1 text-sm leading-snug font-semibold">
                                                {{ is_array($val) ? json_encode($val) : $val }}</p>
                                        </li>
                                    @endforeach
                                @endif
                            </ul>
